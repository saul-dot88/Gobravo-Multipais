defmodule BravoMultipaisWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BravoMultipaisWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      BravoMultipaisWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "authenticates user on subsequent requests when remember_me is set", %{
      conn: conn,
      user: user
    } do
      # 1) Usuario se loguea con remember_me
      logged_in_conn =
        conn
        |> fetch_cookies()
        |> UserAuth.log_in_user(user, %{"remember_me" => "true"})

      # Guardamos la sesión que dejó ese login
      session = get_session(logged_in_conn)

      # 2) Simulamos una nueva request reutilizando esa sesión
      conn =
        logged_in_conn
        |> recycle()
        |> Map.replace!(:secret_key_base, BravoMultipaisWeb.Endpoint.config(:secret_key_base))
      ...
      # Hay un token de sesión activo
      assert get_session(conn, :user_token)
      # Sigue marcado remember_me
      assert get_session(conn, :user_remember_me)
      # Y hay live_socket_id
    ...
    test "keeps the same token after a few days", %{conn: conn, user: user} do
      # 1) Login inicial con remember_me
      logged_in_conn =
        conn
        |> fetch_cookies()
        |> UserAuth.log_in_user(user, %{"remember_me" => "true"})

      # Token de sesión y cookie generados en el login
      token = get_session(logged_in_conn, :user_token)
      %{value: signed_token} = logged_in_conn.resp_cookies[@remember_me_cookie]

      # 2) Marcamos el token como viejo en la BD (10 días atrás)
      offset_user_token(token, -10, :day)

      # Volvemos a leer el usuario desde Accounts para tener el authenticated_at actual
      user_from_db = Accounts.get_user_by_session_token(token)

      # 3) Simulamos una nueva request con ese token y cookie
      conn =
        conn
      ...
      # ✅ El token de sesión se mantiene (tu implementación no lo re-emite)
      assert get_session(conn, :user_token) == token

      # ✅ Sigue activo el flag remember_me
      assert get_session(conn, :user_remember_me)
    end
  end
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: BravoMultipaisWeb.Endpoint,
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      ...
      # Genera el token para un usuario cuya authenticated_at ya es "vieja"
      user_token = Accounts.generate_user_session_token(user)

      # (Opcional, pero útil para asegurarnos de que se guardó bien en la BD)
      user_from_db = Accounts.get_user_by_session_token(user_token)
      assert user_from_db.authenticated_at == eleven_minutes_ago
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BravoMultipaisWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
